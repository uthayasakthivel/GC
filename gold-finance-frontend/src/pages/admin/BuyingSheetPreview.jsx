// src/pages/sheets/BuyingSheetPreview.jsx
...
export default function BuyingSheetPreview({ sheetType }) {
  const { id } = useParams();
  console.log(sheetType, "mmmmmmm");
  const printRef = useRef();
  const { sheet, loading, error } = useSheetDetails("buying", id);
  const { handlePrint } = usePrintSheet(printRef);
  const { handleDownloadPDF } = useDownloadPDF(
    printRef,
    sheet ? `BuyingSheet-${sheet.sheetNumber}.pdf` : "sheet.pdf"
  );
